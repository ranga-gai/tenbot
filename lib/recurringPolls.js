 ...
 * and automatic posting of recurring tennis match polls (daily or on specific days of the week).
 */
...
async function cancelRecurringPoll({ recurringPolls, persistPolls, sender, scheduleId }) {
  ...
  return `🗑️ Cancelled and deleted recurring poll schedule \`${cleanId}\` (Match time: ${sched.matchDisplay} ${sched.daysDisplay || ''}).`;
}
...
async function pauseRecurringPoll({ recurringPolls, persistPolls, scheduleId }) {
  ...
  sched.enabled = false;
  persistPolls();
  return `⏸️ Paused recurring poll schedule \`${cleanId}\` (${sched.matchDisplay}). Use "!resumerecurringpoll ${cleanId}" to re-enable.`;
}
...
async function resumeRecurringPoll({ recurringPolls, persistPolls, scheduleId }) {
  ...
  sched.enabled = true;
  persistPolls();
  return `▶️ Resumed recurring poll schedule \`${cleanId}\` (${sched.matchDisplay}). It will post at ${sched.postDisplay} on ${sched.daysDisplay || 'scheduled days'}.`;
}
