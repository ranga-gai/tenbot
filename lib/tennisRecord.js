...
async function fetchTennisRecordRating(name) {
  ...
  try {
    ...
    // When both first and last names exist, query search.aspx to check for multiple players and pick San Jose, CA
        ...
        if (searchRes.ok) {
          ...
          if (candidates.length > 0) {
            ...
            if (chosen?.href) {
              ...
              targetProfileUrl = `https://www.tennisrecord.com${path}`;
    ...
    if (!targetProfileUrl) {
      targetProfileUrl = `https://www.tennisrecord.com/adult/profile.aspx?playername=${encodeURIComponent(cleanName)}`;
    }
    ...
    return parseProfileRating(html);
  } catch (err) {
