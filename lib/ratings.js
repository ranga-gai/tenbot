 ...
 * Also automatically migrates any legacy '@s.whatsapp.net' keys in ratings.json
 * to their '@lid' keys and cleans up any redundant internal 'id' or 'lid' fields.
 */
function syncRatingsWithNames() {
  ...
  for (const [key, p] of players) {
    ...
    // Clean up internal id/lid fields
    if (p.id !== undefined || p.lid !== undefined) {
      delete p.id;
      delete p.lid;
      ratingsChanged = true;
    }

    const cleanId = key.includes('@s.whatsapp.net') ? key : null;
    const cleanLid = key.includes('@lid') ? key : null;
    const canonicalKey = cleanLid || namesStore.resolveCanonicalId(cleanId || key) || cleanId || key;

    // If key was @s.whatsapp.net and LID is known, migrate entry
    if (canonicalKey !== key && canonicalKey.endsWith('@lid')) {
      ratingsData.players[canonicalKey] = p;
      delete ratingsData.players[key];
      ratingsChanged = true;
    }

    const primaryKey = canonicalKey;
    if (!namesData[primaryKey]) {
      const displayName = namesStore.getName(primaryKey) || (primaryKey.includes('@') ? (primaryKey.split('@')[0].replace(/\D/g, '').slice(-4) ? `Player (${primaryKey.split('@')[0].replace(/\D/g, '').slice(-4)})` : primaryKey) : primaryKey);
      const pn = primaryKey.endsWith('@s.whatsapp.net') ? primaryKey : namesStore.getPnByLid(primaryKey);
      namesData[primaryKey] = {
        name: displayName,
        pn: pn || null,
        aliases: []
      };
  ...
  if (ratingsChanged) {
    save(ratingsData);
    console.log('[ratings] Migrated legacy phone number keys to LID keys in ratings.json.');
  }
...
/**
 * Migrates any entries in ratings.json keyed by phone numbers to their LID keys.
 */
function deduplicateRatingsWithLidMap(pnToLidMap) {
  if (!pnToLidMap || !(pnToLidMap instanceof Map)) return;
  const ratingsData = load();
  let changed = false;

  for (const [pn, lid] of pnToLidMap.entries()) {
    if (ratingsData.players[pn]) {
      const entry = ratingsData.players[pn];
      delete entry.id;
      delete entry.lid;
      ratingsData.players[lid] = entry;
      delete ratingsData.players[pn];
      changed = true;
    }
  }
  ...
  if (changed) {
    save(ratingsData);
    console.log('[ratings] Migrated legacy phone number entries to LID keys in ratings.json');
  }
}
...
function setHundredths(ratingsData, idOrName, hundredths, extra = {}) {
  ...
  // Sync names.json (keyed by LID)
  if (primaryKey && name && !isPlaceholder(name)) {
    namesStore.setName(primaryKey, name, extra.aliases || [], extra.pn || (cleanJid && cleanJid.endsWith('@s.whatsapp.net') ? cleanJid : null));
  }
  ...
  if (match) {
    ...
    // Clean up internal id/lid fields
    delete match.entry.id;
    delete match.entry.lid;

    // If key was PN and LID is now known, migrate to LID key
    if (cleanLid && match.key !== cleanLid) {
      ratingsData.players[cleanLid] = match.entry;
      delete ratingsData.players[match.key];
    }
  } else {
...
async function ensureRated(players, jidMap = new Map()) {
  ...
  for (const { name, jid, lid, pn } of missing) {
    ...
    if (!existing) {
      const now = new Date().toISOString();
      setHundredths(current, lid || jid || name, toHundredths(initialVal), {
        name,
        jid,
        lid,
        pn,
...
async function refreshPeriodicRatings({ force = false, delayMs = 500 } = {}) {
  ...
  for (const [key, p] of players) {
    const playerName = namesStore.getName(key);
