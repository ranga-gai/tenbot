 ...
 * Also cleans up any legacy 'id' or 'lid' fields inside ratings.json player objects.
 */
function syncRatingsWithNames() {
  ...
  for (const [key, p] of players) {
    ...
    // Clean up legacy id/lid fields in ratings object
    ...
    const primaryKey = namesStore.resolveCanonicalId(key);
    if (primaryKey !== key) {
      ratingsData.players[primaryKey] = p;
    ...
    if (!namesData[primaryKey]) {
      ...
      namesData[primaryKey] = {
        name: displayName,
        pn: primaryKey.endsWith('@s.whatsapp.net') ? primaryKey : null,
        aliases: []
      };
      createdCount++;
    }
  ...
  if (ratingsChanged) {
    save(ratingsData);
    console.log('[ratings] Cleaned up legacy id/lid fields in ratings.json.');
  }
 ...
 * Ratings object holds only rating and metadata (no id field).
 */
function setHundredths(ratingsData, idOrName, hundredths, extra = {}) {
  ...
  if (match) {
    ...
    // Clean up any legacy id or lid field
...
async function refreshPeriodicRatings({ force = false, delayMs = 500 } = {}) {
  ...
  // Include all players from names.json
  const nameEntries = namesStore.getAllEntries();
  for (const entry of nameEntries) {
    if (!entry.name || isPlaceholder(entry.name)) continue;
    const lid = entry.id;
    const match = findPlayer(ratingsData, entry.name, lid);
    if (!match) {
