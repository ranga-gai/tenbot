 ...
 * If an entry is missing, creates it with their name / LID and empty aliases.
 */
function syncRatingsWithNames() {
  ...
  for (const [key, p] of players) {
    if (!p) continue;
    const cleanId = (p.id || (key.includes('@s.whatsapp.net') ? key : null))?.trim();
    const cleanLid = (p.lid || (key.includes('@lid') ? key : null))?.trim();
    const cleanName = (p.name || (!key.includes('@') ? key : null))?.trim();

    // Prefer LID as primary key
    const primaryKey = cleanLid || namesStore.resolveCanonicalId(cleanId || key) || cleanId || key;
    if (!primaryKey && !cleanName) continue;

    if (!namesData[primaryKey]) {
      const displayName = cleanName || (primaryKey.includes('@') ? (primaryKey.split('@')[0].replace(/\D/g, '').slice(-4) ? `Player (${primaryKey.split('@')[0].replace(/\D/g, '').slice(-4)})` : primaryKey) : primaryKey);
...
function setHundredths(ratingsData, idOrName, hundredths, extra = {}) {
  ...
  const cleanLid = extra.lid || (cleanJid ? namesStore.resolveCanonicalId(cleanJid) : null);
  const match = findPlayer(ratingsData, idOrName, cleanJid || cleanLid);
  const newRating = fromHundredths(clampH(hundredths));
  const now = new Date().toISOString();

  const primaryKey = cleanLid || (match ? match.key : (cleanJid || keyFor(name || idOrName)));

  // Sync names.json (single canonical LID entry)
  ...
  if (match) {
    ...
    if (cleanLid && !match.entry.lid) match.entry.lid = cleanLid;
  ...
  try {
    ...
    if (knownNames instanceof Map) {
      for (const [jid, name] of knownNames.entries()) {
        ...
        const canonicalJid = pn || normJid;
        namesStore.setName(canonicalJid, name);
