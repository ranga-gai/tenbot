 ...
 * at INITIAL_RATING for anyone we haven't rated yet. A pairing's rating is the
 * sum of its two players', so an even court is one where the two sums are
...
/**
 * Writes the starting rating for any of these players we haven't rated yet, so
 * a player shows up in "!ratings" from their first session rather than only
 * after their first recorded score.
 */
function ensureRated(names) {
  const ratings = load();
  const missing = names.filter((name) => !ratings.players[keyFor(name)]);
  if (missing.length === 0) return 0;

  for (const name of missing) setHundredths(ratings, name, INITIAL_H);
  save(ratings);
