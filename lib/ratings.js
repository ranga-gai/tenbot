...
function setHundredths(ratings, name, hundredths) {
  if (isPlaceholder(name)) return; // Do not record placeholder player names in ratings
 ...
 * to INITIAL_RATING (3.49) if not found. Does not rate generic placeholder players.
 */
async function ensureRated(names) {
  const rBefore = load();
  const missing = names.filter((name) => !rBefore.players[keyFor(name)] && !isPlaceholder(name));
  ...
  for (const name of missing) {
    ...
    const current = load();
    if (!current.players[keyFor(name)]) {
      setHundredths(current, name, toHundredths(initialVal));
      save(current);
    }
  }
...
function applyResult(winners, losers, sets) {
  ...
  for (const name of [...winners, ...losers]) {
    before[name] = fromHundredths(hundredthsFor(ratings, name));
  }

  const setResults = [];
  for (const [gamesA, gamesB] of sets) {
    const res = applySet(ratings, winners, losers, gamesA, gamesB);
  ...
  const changes = [...winners, ...losers]
    ...
    .map((name) => {
      ...
      const to = fromHundredths(hundredthsFor(ratings, name));
      return { name, from, to };
    });
...
module.exports = {
  ...
  pairRating,
  applyResult,
  formatRating: (r) => (typeof r === 'number' ? r.toFixed(2) : String(r))
};
