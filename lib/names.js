...
function setFullName(id, fullName) {
  ...
  const existing = store[targetKey] || { name: cleanFull || 'Player', fullName: cleanFull || null, pn: null, aliases: [] };
  existing.fullName = cleanFull || null;
...
function setName(id, name, aliases = [], pn = null, fullName = null) {
  ...
  else if (existing.fullName === undefined) existing.fullName = null;
...
function setMapping(lid, pn, name = null, fullName = null) {
  ...
  const entry = store[cleanLid] || {
    name: existingName || 'Player',
    fullName: existingFullName || null,
    pn: cleanPn,
    aliases: []
  };
  ...
  else if (entry.fullName === undefined) entry.fullName = null;
...
function addAliasForPlayer(nameOrId, alias, optionalJid = null) {
  ...
  if (match) {
    ...
    return { id: match.id, name: existing.name, fullName: existing.fullName || null, aliases: existing.aliases, pn: existing.pn || null };
  }
  ...
  save(store);
  return { id: primaryKey, name: cleanTarget, fullName: null, aliases: [cleanAlias], pn: newEntry.pn };
}
...
function removeAliasForPlayer(nameOrId, alias) {
  ...
  if (cleanTarget) {
    ...
    if (entry.aliases.length !== beforeLen) {
      save(store);
      return { id: match.id, name: entry.name, fullName: entry.fullName || null, aliases: entry.aliases, pn: entry.pn || null, removed: cleanAlias };
    }
    return null;
  }
  ...
  for (const [id, entry] of Object.entries(store)) {
    if (Array.isArray(entry.aliases)) {
      ...
      if (entry.aliases.length !== beforeLen) {
        save(store);
        return { id, name: entry.name, fullName: entry.fullName || null, aliases: entry.aliases, pn: entry.pn || null, removed: cleanAlias };
      }
    }
  }
