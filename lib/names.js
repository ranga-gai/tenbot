...
// In-memory store loaded once on startup
...
function save(data = null) {
  ...
  try {
    fs.writeFileSync(NAMES_FILE, JSON.stringify(store || {}, null, 2));
  } catch (err) {
 ...
 */
function resolveCanonicalId(id, customStore = null) {
  const cleanId = cleanStr(id);
  if (!cleanId) return null;
  if (cleanId.endsWith('@lid')) {
    return cleanId; // already LID
  }
  const currentStore = customStore || store;
 ...
 */
function getLidByPn(pn, customStore = null) {
 ...
 */
function getPnByLid(lid, customStore = null) {
 ...
 */
/**
 * Resolves any name, canonical display name, alias, JID, or phone number to the player's full name.
 * If names.json has a fullName with multiple words, returns it.
 * If fullName is missing or single word, checks ratings.json for tennisRecordUrl playername.
 * If not found, returns the cleaned original name.
 */
function resolveFullName(nameOrAlias) {
  if (!nameOrAlias || typeof nameOrAlias !== 'string') return nameOrAlias;
  const clean = nameOrAlias.trim();
  if (!clean) return clean;

  const match = findIdByNameOrAlias(clean);
  if (match) {
    const entry = match.entry;
    if (entry) {
      if (entry.fullName && entry.fullName.trim().includes(' ')) {
        return entry.fullName.trim();
      }
      // Check ratings.json for tennisRecordUrl playername
      try {
        const ratingsFile = path.join(__dirname, '..', 'ratings.json');
        if (fs.existsSync(ratingsFile)) {
          const ratingsData = JSON.parse(fs.readFileSync(ratingsFile, 'utf8'));
          const r = ratingsData.players && (ratingsData.players[match.id] || (entry.pn && ratingsData.players[entry.pn]));
          if (r && r.tennisRecordUrl) {
            const pMatch = r.tennisRecordUrl.match(/playername=([^&]+)/i);
            if (pMatch) {
              const decoded = decodeURIComponent(pMatch[1].replace(/\+/g, ' ')).trim();
              if (decoded.includes(' ')) {
                return decoded;
              }
            }
      ...
      return entry.fullName || entry.name || clean;
    }
  }

  // Check if clean matches any tennisRecordUrl playername directly
  try {
    const ratingsFile = path.join(__dirname, '..', 'ratings.json');
    if (fs.existsSync(ratingsFile)) {
      const ratingsData = JSON.parse(fs.readFileSync(ratingsFile, 'utf8'));
      for (const [, r] of Object.entries(ratingsData.players || {})) {
        if (r && r.tennisRecordUrl) {
          const pMatch = r.tennisRecordUrl.match(/playername=([^&]+)/i);
          if (pMatch) {
            const decoded = decodeURIComponent(pMatch[1].replace(/\+/g, ' ')).trim();
            if (keyFor(decoded) === keyFor(clean) || keyFor(decoded).startsWith(keyFor(clean) + ' ')) {
              return decoded;
            }
          }
        }
