...
/**
 * Synchronizes pn, names, and aliases from poll store maps during startup.
 */
function syncFromPollStore({ lidToPn, pnToLid, knownNames } = {}) {
  const store = load();
  let changed = false;

  const mappings = [];
  if (lidToPn instanceof Map) {
    for (const [lid, pn] of lidToPn.entries()) mappings.push({ lid: String(lid).trim(), pn: String(pn).trim() });
  }
  if (pnToLid instanceof Map) {
    for (const [pn, lid] of pnToLid.entries()) mappings.push({ lid: String(lid).trim(), pn: String(pn).trim() });
  }

  for (const { lid, pn } of mappings) {
    if (!lid || !pn) continue;
    let entry = store[lid] || { name: null, pn: pn, aliases: [] };
    if (!entry.pn || entry.pn !== pn) {
      entry.pn = pn;
      changed = true;
    }

    // Merge legacy phone number entry into LID entry if present
    if (store[pn]) {
      const pnEntry = store[pn];
      if (!entry.name && pnEntry.name) entry.name = pnEntry.name;
    ...
    if (!entry.name && knownNames instanceof Map) {
      entry.name = knownNames.get(lid) || knownNames.get(pn) || null;
    }
  ...
  if (knownNames instanceof Map) {
    for (const [jid, name] of knownNames.entries()) {
      ...
      const lid = cleanJid.endsWith('@lid') ? cleanJid : (pnToLid instanceof Map ? pnToLid.get(cleanJid) : null);
      const pn = cleanJid.endsWith('@s.whatsapp.net') ? cleanJid : (lidToPn instanceof Map ? lidToPn.get(cleanJid) : null);

      if (lid) {
        const entry = store[lid] || { name, pn: pn || null, aliases: [] };
        if (!entry.name) entry.name = name;
        if (pn && !entry.pn) entry.pn = pn;
        store[lid] = entry;
        if (store[pn]) delete store[pn];
        changed = true;
      }
    }
  }
  ...
  if (changed) {
    save(store);
    console.log('[names] Synced pn and names from poll store into names.json');
  }
}
...
module.exports = {
  ...
  findIdByNameOrAlias,
  syncFromPollStore,
  deduplicateWithPnMap: (pnToLid) => syncFromPollStore({ pnToLid }),
  getAllEntries,
  resolveCanonicalId,
  keyFor
};
