...
function resolvePlayDateTime(dayWord, timeWord, now = new Date()) {
  ...
  if (!hasExplicitDay && hasTime && targetDate.getTime() <= now.getTime()) {
    const temp = new Date(Date.UTC(targetYear, targetMonth, targetDay + 1));
    targetDate = createSanJoseDate(temp.getUTCFullYear(), temp.getUTCMonth(), temp.getUTCDate(), targetDate.getUTCHours() - (targetDate.getTimezoneOffset() / 60), minute, 0);
    // Directly re-resolve for tomorrow with the same hour/minute
